...
class PINN_Trainer:
    def __init__(self, config_dir, model, train_loader, val_loader, test_loader, optimizer, criterion,
        ...
        self.mse_mape_save_path = f"{self.config_dir}/pinn_mse_mape.txt"
    ...
    def _save_checkpoint(self, epoch, loss):
        ...
        if loss < self.best_checkpoint_loss:
            checkpoint_path = os.path.join(
                self.checkpoint_dir,
                f"pinn_checkpoint_epoch_{epoch + 1}_loss_{loss:.6f}.pth"
            )
    ...
    def export_predictions(self):
        ...
        print(f"💾 Predictions saved at file {self.predictions_save_path}")
