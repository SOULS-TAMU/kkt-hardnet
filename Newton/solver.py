    ...
    def solve(self, initial_guess: dict, parameter_values: dict):
        ...
        for i in range(self.max_iter):
            ...
            try:
                ...
                delta_x = np.linalg.solve(Jx_numpy, -Fx_numpy)
                # Jx_numpyT = Jx_numpy.T
                # JxTJx = Jx_numpyT @ Jx_numpy
                # JxTJx += self.reg_factor * np.eye(JxTJx.shape[0])
                # rhs = - Jx_numpyT @ Fx_numpy
                # delta_x = np.linalg.solve(JxTJx, rhs)
            Fx_norm = np.linalg.norm(Fx_numpy)
            # delta_x_norm = np.linalg.norm(delta_x)
            # self.norm_list.append(delta_x_norm)
            self.functional_evaluations.append(np.linalg.norm(Fx_numpy))

            # # Step 4: Check convergence
            # if delta_x_norm < self.tol:
